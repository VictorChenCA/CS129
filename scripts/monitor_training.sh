...
while true; do
    ...
    tail -3 "${LOG_FILE:-singletask_training.log}" 2>/dev/null || echo "Log file not found yet"
    echo ""
    echo "=== Checking for errors ==="
    tail -10 "${LOG_FILE:-singletask_training.log}" 2>/dev/null | grep -i "error\|exception\|traceback" | tail -3 || echo "No errors found"
    echo ""
